 ...
 */
export const registerPlayerHandlers = ({ socket, gameService, roomManager, broadcaster }) => {
  ...
  socket.on(SocketEvents.PLAYER_JOIN, (payload = {}, cb) => {
    ...
    if (!room) return ack(cb, { ok: false, code: 'NOT_FOUND', message: 'Room not found' });

    const name = sanitizeUsername(payload.name);
    if (!name) return ack(cb, { ok: false, code: 'BAD_NAME', message: 'Invalid display name' });

    const { player, error, code } = gameService.joinRoom({ room, name, socketId: socket.id });
    ...
    ack(cb, { ok: true, playerId: player.id, code: room.code, state: room.state });
    broadcaster.roomState(room);
    broadcaster.toSocket(room.hostSocketId, SocketEvents.PLAYER_JOINED, player.toPublic());
  });
