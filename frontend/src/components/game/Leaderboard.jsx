    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          {isWinner && (
            <Trophy className="size-4 text-primary shrink-0" aria-hidden />
          )}
          <span className="font-body text-sm text-text truncate">
            {entry.name}
          </span>
          {isYou && (
            <span className="font-label text-[10px] text-secondary">YOU</span>
          )}
          {hasSubmitted && (
            <CheckCircle2
          ...
          {entry.connected ? (
            ...
            <WifiOff
              className="size-3 text-danger/60 shrink-0"
              aria-label="disconnected"
            />
          )}
        </div>
      </div>
      {!hideScores && entry.roundScore != null && (
